                    ...
                    <div class="gs-card">
                        ...
                        <div class="gs-input-group">
                            ...
                            <textarea v-model="form.customer_problem_description" class="gs-input" rows="3"
                                placeholder="مشتری چه مشکلی گزارش داده..." style="resize:vertical"></textarea>
                        </div>
                        <div class="gs-input-group">
                            <label class="gs-input-label">تشخیص فنی</label>
                            <textarea v-model="form.diagnosis_description" class="gs-input" rows="3"
                                placeholder="نتیجه بررسی فنی..." style="resize:vertical"></textarea>
                        </div>
                    </div>
                </div>

                <!-- Left column: status/dates + service types + items -->
                <div style="display:flex;flex-direction:column;gap:1rem">
                    ...
                    <button type="submit" class="gs-btn gs-btn-primary gs-btn-lg" :disabled="form.processing"
                        style="width:100%;justify-content:center">
                        {{ form.processing ? 'در حال ثبت...' : 'ثبت سرویس' }}
                    </button>
                </div>
            </div>
                    ...
                    <div class="gs-modal-body">
                        ...
                        <template v-else>
                            ...
                            <button v-for="item in itemsInCategory" :key="item.id" type="button" class="gs-picker-item"
                                @click="pickItem(item)">
                                <span>{{ item.name }}</span>
                                <span class="gs-muted">{{ formatPrice(item.price) }}</span>
                            </button>
                        </template>
                    </div>
                </div>
...
// 3) واچرهای وابسته به props/form
watch(() => form.customer_id, (newCustomerId) => {
    const selected = props.requests.find(r => r.id === form.request_id)
    if (selected && selected.customer_id !== newCustomerId) {
        form.request_id = ''
    }
})

watch(() => form.request_id, (newRequestId) => {
    const selected = props.requests.find(r => r.id === newRequestId)
    if (selected) {
...
function pickItem(item) {
    form.items.push({ item_id: item.id, quantity: 1, unit_price: item.price })
    closePicker()
}
...
// --- quick create service type ---
const serviceTypesList = ref([...props.serviceTypes])
...
/* Modals (theme-aware, reuse gs-card) */
.gs-modal-overlay {
    position: fixed;
    inset: 0;
