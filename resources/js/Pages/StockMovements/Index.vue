<template>
    <AppLayout>
        <div class="gx-page">
            ...
            <!-- ================= هیرو ================= -->
            <LuxHero
                chip="ماژول گردش انبار"
                chip-two="Stock Movements"
                title="گردش «انبار»"
                lead="ورود و خروج اقلام انبار — رهگیری کامل هر واحد کالا با شماره سریال و گارانتی."
                cube="🔄"
                satellite="📦"
                :stats="[
            ...
            >
                <template #chip-icon><Repeat :size="13" /></template>
                <template #actions>
            ...
            <!-- ================= خلاصه موجودی هر قلم ================= -->
            <div class="gx-stockgrid" v-if="stockSummary.length">
                <article
                    v-for="(item, idx) in stockSummary"
                    :key="item.id"
                    v-tilt="{ max: 6, scale: 1.015, lift: 10 }"
                    class="gx-stockcard a3d-aura"
                ...
                >
                    ...
                    <p class="gx-stockcard__price">قیمت: <b class="gx-price">{{ formatPrice(item.price) }}</b></p>
            ...
            <section class="gx-panel" :style="{ '--gx-i': 2 }" style="margin-top: 1.3rem">
                <div class="gx-panel__head">
                    <span class="gx-panel__icon"><History :size="16" /></span>
                    <div>
                ...
                <table class="gs-table" v-if="movements.data?.length">
                    ...
                    <tbody>
                        <tr v-for="m in movements.data" :key="m.id">
                            ...
                            <td>
                                <span class="gx-status" :class="isIn(m.movement_type) ? 'gx-status--green' : 'gx-status--red'">
                                    <component :is="isIn(m.movement_type) ? ArrowDownToLine : ArrowUpFromLine" :size="12" />
                                    {{ moveLabel(m.movement_type) }}
                                </span>
                            </td>
                            <td :style="{ color: isIn(m.movement_type) ? 'var(--gs-success)' : 'var(--gs-error)', fontWeight: 800 }">
                    ...
                    <div class="gs-modal">
                        ...
                        <div class="gs-input-group" v-if="selectedItemHasSerial && isInboundType">
                            <label class="gs-input-label">
                                شماره سریال (اختیاری)
                                <span class="gx-labelhint">— {{ faInt(enteredSerials.length) }} از {{ faInt(moveForm.quantity || 0) }} وارد شده</span>
                            </label>
                        ...
                        <div class="gs-input-group" v-if="selectedItemHasSerial && !isInboundType">
                            <label class="gs-input-label">
                                ...
                                <span class="gx-labelhint">— حداکثر {{ faInt(moveForm.quantity || 0) }} مورد؛ باقی خودکار کسر می‌شود</span>
                            </label>
                        ...
                        <div style="display:flex;gap:.75rem;justify-content:flex-end;margin-top:.7rem">
                            <button type="button" @click="showForm = false" class="a3d-btn a3d-btn--ghost">انصراف</button>
                            <button type="button" @click="submitMove" class="a3d-btn a3d-btn--gold" :disabled="moveForm.processing">
                                <Plus :size="14" />
